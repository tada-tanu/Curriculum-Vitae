...
- 物事を論理的に整理する事が得意。
- 人に何かを教えたり説明する事が得意。1対1でも、1対多の状況でも。
- バックエンド、ネイティブアプリ、データ加工・可視化の経験がある。
...
#### おサイフケータイ残高読取機開発 (2019/10 ~ 2019/12)

- 要旨: 仕入れた中古AndroidのFelicaを読み取り、おサイフケータイの残高を確認するIoT機器の開発主担当を担いました。(従来の残高読取ソフトは1件100円程するため、内製化により大幅なコストカットへ繋がります。)
- ポジション: 開発スタッフ (チーム 3名)
- 使用技術: LAMP(Raspbian, Nginx, MariaDB, Python), RaspberyPi, ShellScript 

#### 社内用iOSレジアプリ運用・保守 (2019/8 ~ 2020/3)

- 要旨: Webツールの他、販売店舗のレジをiOSアプリで内製していました。こちらはアプリチームが別途あり、私も参加していました。私が参画したタイミングではほとんどの機能は開発済みで、運用・保守がメインでした。
    - 開発例①: オフライン時にも販売が出来るよう、Realmを使用したアプリ内データベースの構築
    - 開発例②: Alamofireを用いてのAPI追加

- ポジション: 開発スタッフ (チーム 2名)
- 使用技術: Swift, XCode, Alamofire, Git

#### 社内Webツールの開発・運用・保守 (2019/3 ~ 2020/12)

- 要旨: 社内SEチームの主な業務は社内に配布しているWebツールの開発・運用・保守でした。特にツール別に担当がいるわけではなく、チーム全員でツールの開発を行います。ツールはスマホの販売・買取を補助するものがほとんどで、社内ネットワークにデプロイすることで各拠点のスタッフはブラウザからツールを利用する事ができます。下記にツール例を記載します。
    - ツール例①: 競合他社買取・販売価格スクレイピングツール
    - ツール例②: スマホのネットワーク利用制限を判別するツール
    - ツール例③: スマホ自動査定機IPAS
- ポジション: 開発スタッフ (チーム 6名)
- 使用技術: LAMP(CentOS, Apache, MySQL, PHP), ShellScript, HTML/CSS/JS, Git
